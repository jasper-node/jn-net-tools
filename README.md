...
_This generates `jnnt-aarch64.so` (Linux), `.dylib` (macOS), or `.dll` (Windows) in `lib/`_

Cross-compile by passing a Rust triple:
...
rather than `cargo build` directly, or you will re-mint an unfloored library.
